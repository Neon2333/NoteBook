...
使用Nagle算法时，会将多个小数据包合并后一起发送。只有收到上次发送数据包的ACK时才发送下一个数据包，可以明显减少数据包的个数。在未收到上次数据的ACK时，数据将会存储在输出缓冲区。
...
客户端ctrl-c后，子进程return，但此时由于主进程已经销毁了，没有执行`child_proc`，但奇怪的是，子进程也没有变成僵尸进程，子进程也销毁了。

#### Q：不是说父进程结束时，子进程也会结束吗？
