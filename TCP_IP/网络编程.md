...
>
> oldact——获取存储着上一个Handler函数的结构体的地址
...
void child_proc(int sig){
    int status;
    if(sig == SIGCHLD){
        pdt_t pid = waitpid(-1, &status, WNOHANG);
        if(WIFEXIST(status)){
            cout<<"child proc id= "<<pid<<", child proc return val= "<<WEXISTSTATUS(status)<<endl;
        }
    }
}
...
待连接上后，accept提取一个连接，就fork一个子进程，处理相应请求。
