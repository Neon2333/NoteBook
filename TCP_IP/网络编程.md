...
因此，当某一端在发送完数据需要断开连接后，若还需要接收对端的数据或者向对端发数据，就需要只“关闭一半的连接”，使得断开连接后还能收或发数据。
...
>
> ***注意：h_addr_list指向的数组实际保存的是in_addr类型变量的地址，而不是char类型地址（主要是为了兼容IPv6）。所以在读取h_addr_list时要强转IP addr #的地址，改变读取内存的长度。***
>
...
所以，可以将SO_REUSEADDR设置为1，处于Time-wait状态下的套接字的端口号就可以立即重新被使用。SO_REUSEADDR的默认值是0。
