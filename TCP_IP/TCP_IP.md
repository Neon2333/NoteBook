...
* **在ARP缓存表中未找到对应MAC时ARP协议工作：将自身MAC地址作为源MAC地址，将目的MAC地址设为FF FF FF FF FF FF，封装成MAC帧，在集线器上广播，目标主机收到后返回ARP响应报文**
...
* ARP缓存
  ...
  向某目的主机发送过一次IP数据包的源主机很大可能会继续发送IP数据包，而不会只发送一次。所以在源主机设置ARP缓存可以避免每次都发送ARP请求包。同理，目的端主机也有ARP缓存，接收ARP请求包时会缓存其中的源主机IP和MAC地址，并给源主机发送一个ARP响应包。
...
* ARP工作过程
  ...
  > 4. 某个主机（192.168.1.2）需要发送报文时，首先检查 ARP 列表中是否有对应 IP 地址的目的主机的 MAC 地址，如果有，则直接发送数据，如果没有，就发送一个ARP请求包到广播地址（192.168.1.255） ，该数据包包括的内容有：源主机 IP 地址，源主机 MAC 地址，目的主机的 IP 地址（192.168.1.3）、目的MAC（置为FF FF FF FF FF FF）等。
  ...
  >
  > ​       **（C）** 然后将自己的 MAC 地址写入 ARP 响应包中，单播至源主机。
  >
...
* 静态ARP/动态ARP
  ...
  **静态ARP**——需要管理员手工指定建立AP-MAC映射表，需要管理员手工建立和维护，项存在硬盘中，而不是缓存表，计算机重新启动后仍然存在，且遵循静态优于动态的原则，以这个设置不对，可能导致无法上网。

  **动态ARP**——通过报文去学习ARP表项，不需要管理员手工建立和维护，会出现覆盖和老化的情况。
...
* **动态ARP的老化与更新**
  ...
  同的系统对ARP表的老化时间设定不太一样，思科是 5分钟，华为是 20分钟

  默认情况，WINDOWS系统的ARP缓存中的表项仅存储2分钟。一个ARP缓存表项在2分钟内被用到，则其期限再延长2分钟，直到**最大生命期限10分钟**为止。（***Q：老化时间是指2分钟，还是最大生命期限的10分钟？？？***）超过10分钟的最大期限后，ARP缓存表项将被移出。并且通过另外一个ARP请求——ARP回应交换来获得新的对应关系。ARP缓存表项的存放时间可以通过改变ArpCacheLife和ArpCacheMinReferencedLife的注册表值来重新设置。
