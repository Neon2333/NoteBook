...
* 可靠传输——超时重传机制和等待确认机制
  ...
  >
  > * **等待确认**——序号seq（**通信双方协商，发送端随机产生一个值seq，放入SYN包的序列号字段，将SYN包发给接收端**（[这样可避免黑客很容易猜到序列号而发起攻击](https://blog.csdn.net/qq_40910541/article/details/88760627)）、确认号ack（确认应答超时、确认应答丢失）。

* `滑动窗口`，包括`发送端窗口`、`接收端窗口rwnd`。在实现上即`缓存`，是内存中的一块空间，**发送端**以3个指针维护四个区域。

  连续ARQ协议（***连续发送、累积确认***）——在内存中开辟空间作为“窗口”，用来缓存那些发送端已经发出但是尚未收到确认的包，当收到确认时就将对应的包从缓存中移除。
...
* 流量控制——使用滑动窗口还可以让接收端按照自身的接收窗口rwnd大小，通知发送端，**改变发送端的窗口大小**
...
* 拥塞控制——发送端维护`拥塞窗口cwnd`，发送端取rwnd和cwnd较小者作为发送窗口大小。**慢开始、快速重传、快速恢复**算法设置cwnd长度（字节）。
  ...
  >
  > * **快重传算法/机制**——快重传算法首先要求接收方每收到一个失序的报文段后就立即发出重复确认（为的是使发送方及早知道有报文段没有到达对方），而不要等到收到期望包时再确认（等待确认机制，只规定了收到期望包时才发送ACK）。若接收端接收到**3个**失序包，接收端就会给发送端发送三个期望包的确认告知发送端发送不对，这样发送端的超时计时及时没到时间，也会立即重发接收端需要的包。
  >
  >   ![image-20210722000528357](C:/Users/25224/AppData/Roaming/Typora/typora-user-images/image-20210722000528357.png)
  >
