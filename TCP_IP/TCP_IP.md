...
### （10）IP隧道——在IP报头后继续追加IP报头，以让IPv4数据报可以进入IPv6网络，IPv6进入IPv4的方法
...
### （11）IP任播——将多个功能相同的服务器设成同一个IP，访问该IP时就近分配一个服务器提供连接。如DNS根服务器，不可能只有13台，而是将相同功能的作为同一个IP，且数量更多分布全球不同地点。
