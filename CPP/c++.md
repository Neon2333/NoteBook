...
* std::result_of
  ...
  `std::result_of` 在 C++17 中已被弃用，推荐使用 `decltype(auto)` 来替代（C++14新增特性，无需用decltype显式指出返回值类型）。

  C++11 引入的一个模板类，用于获取函数或可调用对象的返回类型。

  `typename`关键字用于指示编译器将一个**表达式（不仅仅可以是个T）解析为类型**。

  ```cpp
  template <typename F, typename... Args>
  //functype为函数类型为F的函数f传入类型为Args的参数包后，返回值的类型
...
> 与 `decltype` 类似，`decltype(auto)` 也用于编译时自动推导表达式的类型。但是`decltype`需要显式指定表达式供类型推导。
>
> 然而，`decltype(auto)` 提供了一种更简洁、更安全的方式来处理返回类型为 `auto` 的函数，直接`decltype(auto)`就能自动推导出放在的那个位置的类型。
>
> 通过使用 `decltype(auto)`，编译器可以自动推导出函数返回值的类型，而无需显式指定返回类型。这在编写泛型代码和元编程时非常有用，因为它减少了模板元编程的复杂性，并提高了代码的可读性和可维护性。

```cpp
template <typename F, typename... Args>
//->不直接decltype(auto)是因为希望enqueue返回一个future
//所以->跟上future，但是future的模版类型用decltype(auto)自动推导不用管
auto enqueue(F&& f, Args&&... args) -> std::future<decltype(auto)> {
    using return_type = decltype(f(std::forward<Args>(args)...));
    /*auto task = std::make_shared<std::packaged_task<return_type()>>(std::bind(std::forward<F>(f), std::forward<Args>(args)...));*/
    
    std::packaged_task<return_type()> pt(std::bind(std::forward<F>(f), std::forward<Args>(args)...));
    auto task = std::make_shared<std::packaged_task<return_type()>>(pt);
...
> 函数返回的临时对象不是函数体内的那个局部变量。

在C++中，如果函数中定义了局部变量并直接返回它，那么这个局部变量会在函数返回时被销毁。然而，由于返回类型需要生成一个值，所以会调用这个局部变量的拷贝构造函数来创建一个临时变量，这个临时变量就是函数的返回值 。

具体来说，当函数返回局部变量时，会发生以下步骤：

1. **创建局部变量**：在函数内部，首先会创建一个局部变量a。这个局部变量只在函数的作用域内存在，一旦函数执行完毕，这个局部变量就会被销毁。
2. **生成返回值**：然后，为了生成函数的返回值，会调用这个局部变量的**拷贝构造函数**来创建一个**临时变量A()**。这个临时变量的内容和局部变量是一样的。
3. **返回临时变量**：最后，函数会返回这个临时变量。在上层调用这个函数的地方，可以得到这个临时变量的值。
4. **拷贝构造发生**：当上层调用者接收到这个临时变量的值后，**如果将其赋值给一个非引用类型的本地变量**，则会再次通过**拷贝构造函数**进行构造。

**但是，VS等编译器会进行优化，导致构造只被调用1次。实际上，调用了2次拷贝构造。**（如果编译器启用了返回值优化（Return Value Optimization，RVO）或者命名返回值优化（Named Return Value Optimization，NRVO），编译器可能会消除不必要的拷贝构造函数调用。）

需要注意的是，拷贝构造的发生取决于上层调用者的变量类型和接收方式。如果上层调用者使用的是引用类型接收，则不会发生拷贝构造。此外，C++11标准引入了移动语义，如果返回类型支持移动语义并且上层调用者使用了移动构造函数或赋值运算符，那么会发生移动操作而非拷贝构造。
...
* 左值`lvalue`，locator value，可取地址，持久值。
...
* 右值`rvalue`，read value，只读。

  右值有两种：

  **纯右值（常量，无标识符，不能取址）**
  
  **将亡值（曾经是左值，但即将被销毁，但是不能取址。如：函数返回的临时对象、被调用了移动语义的左值**，与纯右值（prvalue）相比，将亡值通常关联着一个已有的对象，而纯右值则是不关联任何对象的临时值。
  
  **右值要用右值引用或万能引用（const左值引用）指向。**
...
> **将亡值类型主要是指生命周期即将结束的值**，这一概念是在C++11标准中引入的。
>
> 它代表了一个对象的生命周期即将结束，并且它的资源**可以**被移动到另一个对象中去。
...
* 常见将亡值：
  ...
  static_cast<T&&>()转换成右值引用后的值

* 应用

  **支持移动语义**：当一个对象是将亡值时，它可以用于初始化或赋值给采用右值引用作为参数的构造函数或赋值运算符，这允许直接"移动"资源而非复制，从而提高程序运行效率。

  **优化资源管理**：通过移动操作，可以避免不必要的对象复制，减少资源消耗，特别是在处理大型数据结构或动态分配内存时尤为重要。
...
* 右值引用：
  ...
  > 函数返回临时对象时，用变量去“接”时，编译器把局部变量通过临时对象通过拷贝函数拷贝给变量，然后销毁临时变量。若临时对象的类型构建起来很耗费资源（时间、内存）。
  >
  > 但如果，函数返回临时对象的右值引用（临时对象是右值），或用右值引用去“接”，都会把临时对象的生命周期延长，并且在临时对象的资源上直接操作，没有拷贝动作。
  >
  > 容器的`push_back()`不会直接把元素放入，而是调用拷贝构造，拷贝了一份放入容器中（现代c++会优先调用移动构造）。emplace_back()是使用**构造对象需要的初始值**直接在容器上构造元素。

  可接收：**常量、临时对象（将亡值的一种）、返回右值的函数返回值**
...
* C风格，函数参数为指针，传入变量地址，在函数内拿到变量地址后对变量进行操作。
...
* 参数和返回值都是变量，用变量来接
  ...
  ```cpp
  A func(int j)	//返回值是A所以要创建临时对象A()
  {
      A a;	//调用A的构造
      a.m_i=j;
      return a;	//调用拷贝创建临时对象A()，内容和a一样
  }
  A b = func(1);	//调用拷贝构造，用临时对象A()创建b
  //construt
  //copy construct
  //copy construct
  ```
...
* 参数和返回值都是变量，用右值引用来接
  ...
  A&& b = func(1);	//通过右值引用指向临时对象A()，不调用拷贝构造
  //construct
  //copy construct
  ```

* 参数和返回值都是右值引用，用右值引用来接
  ...
  A&& func(int j)
  {
      A a;	
      a.m_i=j;
      return std::move(a);	//不创建临时对象，返回指向右值std::move(a)的右值引用
  }
  A&& b = func(1);	//右值引用赋值
  //construct
  ```
...
> 通过拷贝构造函数创建临时变量的方式可能会导致一些性能问题。因为拷贝构造函数需要复制所有的成员变量，如果成员变量很大或者复制操作很复杂，那么这将会消耗很多时间。因此，如果可能，最好避免返回局部变量，而是返回指针或者引用，或者使用移动语义。

**传入构造的参数是个右值引用**，**把这个右值引用的一部分资源（主要是动态分配的堆内存）移动给当前构造出来的这个对象用**，不要再自己开辟资源。
...
> 用临界区把对共享变量访问的代码包住。
>
> **临界区的代码每次只能一个线程拿到锁进入临界区执行，必须要能执行完，不能有while(1)，否则其他线程进不去了。**
>
...
* 只有会被**多个线程**都可能访问到的变量，才是共享变量，才需要加锁。只有1个线程访问的不是共享变量不要加锁。不要乱给变量加锁。

* 加锁-解锁中间的代码块是临界区，每次只有一个线程能***拿到锁***进去执行，一个线程执行时其他线程拿不到锁不能进去执行。

* 让临界区尽量小。

* **锁粒度**：细粒度的锁（即保护的代码段较短）可以减少锁持有的时间，从而减少其他线程等待的时间，但可能会导致更频繁的锁竞争。粗粒度的锁（即保护的代码段较长）虽然减少了锁竞争的次数，但增加了线程等待的时间。

* **避免过度同步**：过度使用互斥锁（例如不必要的锁定或保持锁的时间过长）会导致性能下降。合理设计并发算法和数据结构，以减少锁的使用，是提高多线程程序性能的关键。

* **读写锁**：在某些情况下，可以使用读写锁来提高性能，它允许多个线程同时读取共享数据，只有在写入时才需要独占访问。

  只读的共享变量，不要加锁。

  读取共享变量的代码不要加锁，写入共享变量的代码才加锁。
...
* API
  ...
  ```cpp
  std::future<T> = async(std::launch::async | std::launch::deferred, f, args...);
...
>
> **packaged_task对象自己也可以调用：`std::packaged_task pp(func); pp();`，但要放到线程里执行才是异步的。**
>
> ```cpp
...
> ```
>
> 流程：对可调用对象（函数、lambda、bind()返回值）进行包装，主线程就可通过`get_future()`获取`std::future`。再放到子线程里调用。主线程通过`future.get()`获得返回值。
...
* typename关键字
  ...
  `typename`关键字用于指示编译器将一个**表达式（不仅仅可以是个T）解析为类型**。当使用模板时，编译器需要知道某个表达式的类型才能正确地实例化模板。

  ```cpp
  template <typename F, typename... Args>
  	//auto表示enqueue返回值类型后置，为：future类型，future的模版类型是F(Args...)的返回值类型
  	//typename让编译器知道std::result_of<F(Args...)>::type是类型
  	//std::result_of<F(Args...)>通过result_of获取f传入args执行完后的类型
      auto enqueue(F&& f, Args&&... args) -> std::future<typename std::result_of<F(Args...)>::type> {
          using return_type = typename std::result_of<F(Args...)>::type;	//定义return_type为f返回值类型
          auto task = std::make_shared<std::packaged_task<return_type()>>(std::bind(std::forward<F>(f), std::forward<Args>(args)...));	//task是指向Packaged_task的指针，Packaged_task里面包着f和传入参数args
          std::future<return_type> res = task->get_future();	//res是通过Packaged_task获得的future对象
          {
              std::unique_lock<std::mutex> lock(queue_mutex);
              if (stop) {
                  throw std::runtime_error("enqueue on stopped ThreadPool");
